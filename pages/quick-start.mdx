...
The git history of the `biosnicar-py` repository was rewritten to remove large binary files that had accumulated across past commits, reducing the download size from ~875 MB to ~170 MB. If you cloned the repository before this date your local history will no longer match the remote. To sync up, delete your local clone and re-clone:
...
For programmatic use, `run_model()` is the recommended single entry point. It accepts keyword overrides for any model parameter, runs the full pipeline, and returns an `Outputs` object:
...
Supported override keys: `solzen`, `direct`, `incoming`, `rds`, `rho`, `dz`, `lwc`, `layer_type`, and `impurity_{i}_conc`. `run_model()` is also accessible as `biosnicar.run_model()` after `import biosnicar`.

### Changing input parameters

Most users will want to experiment with changing input parameters.
For batch configuration, adjust the values in the config file `inputs.yaml`.
...
The bio-optical model (in the `biosnicar/biooptical/` module) is generally run separately to the main model code because most users use the default optical properties provided in the downloaded repository.
