...
The core model tests are defined in `test_snicar.py` and the parameter sweep tests in `test_sweep.py`. Fixtures (features that are common to all tests) are defined in `conftest.py`.
To add, remove or refine the automated tests, update these files. It is strongly recommended that these files are not changed and instead any changes are raised as issues for discussion with the repo owners because of their importance for PR quality control.

There are many tests included in `./tests` but they can be divided into three general categories: benchmarking, fuzzing and sweep tests.
...
The `test_sweep.py` file tests the `parameter_sweep` API. These tests verify correct DataFrame shapes for single- and multi-parameter sweeps, physical sanity (e.g. BBA decreases with increasing grain radius or black carbon concentration), spectral output, error handling for unknown parameter keys, and that both the adding-doubling and Toon solvers work correctly through the sweep interface.

## Github Workflow

Any pull-requests to the main branch of BioSNICAR automatically run the benchmarking, fuzzing and sweep tests described above in a virtual machine instance of the PR branch.
