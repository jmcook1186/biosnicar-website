...
`biosnicar` is available as Python code, or it can be run in a web browser using our [app](https://bit.ly/biosnicar). We have designed `biosnicar` to be as flexible, offering multiple options for the radiative transfer solver used, a library of ice refractive indices, illumination profiles and impurity types.
