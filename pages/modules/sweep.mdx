...
`parameter_sweep()` returns a `SweepResult` (a DataFrame subclass) with a `.to_platform()` method that appends band-convolved albedo columns for any supported platform. See the [bands module](/modules/bands) for full documentation.

```py
# Single platform — unprefixed columns
df = parameter_sweep(
    params={"rds": [500, 1000], "solzen": [50, 60]},
).to_platform("sentinel2")
print(df[["rds", "solzen", "BBA", "B3", "NDSI"]])

# Multiple platforms — prefixed columns
df = parameter_sweep(
    params={"rds": [500, 1000]},
).to_platform("sentinel2", "modis")
# Columns: sentinel2_B3, sentinel2_NDSI, ..., modis_B1, modis_NDSI, ...
```
