...
- `file`: name used to look up optical properties from the consolidated `lap.npz` archive (the filename stem is used as a key prefix)
...
- `save_to_lap`: (boolean) if True, saves data into the consolidated `lap.npz` archive
- `impurity_stem`: (string) stem name used as key prefix in `lap.npz`
- `info`: (string) containing any additional metadata (e.g. 'Glacier algae OPs derived from GO calculations with empirical abs_cff')
